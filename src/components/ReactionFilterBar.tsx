...
const ReactionFilterBar = ({active, setActive}: Props) => {
  ...
  return (
    ...
    <ReactionFilterItem src={angry} count={10000} active = {active === 'angry'} state='angry' setActive={setActive}/> */}
      <li className="relative flex cursor-pointer items-center justify-center gap-1 px-4 text-[1rem] font-medium text-[#606366] hover:bg-gray-100" onClick={() => setIsOpenMore((pre) => !pre)}>
        ...
        />
       {
        ...
        {
          isOpenMore && <div className="absolute bottom-0 left-0 w-full translate-y-full rounded-[12px] shadow-[0px_0px_10px_1px_rgba(0_0_0/0.2)] bg-white" onMouseEnter={(e: React.MouseEvent) => e.stopPropagation()}>
          <ul>
            <ModelMoreReaction src={wow} count={10} state='wow' active={active === 'wow'} setActive={setActive}/>
            <ModelMoreReaction src={sad} count={10} state='sad' active={active === 'sad'} setActive={setActive}/>
            <ModelMoreReaction src={angry} count={10} state='angry' active={active === 'angry'} setActive={setActive}/>
          </ul>
        </div>
        }
      </li>
