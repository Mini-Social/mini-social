...
import { Image, X, Smile, Globe, Users, Lock, Check } from 'lucide-react';
import React, {
  useState,
  useRef,
  useEffect,
...
interface Props {
  ...
  isOpenEmoj: boolean;
  setIsOpenEmoj: React.Dispatch<SetStateAction<boolean>>;
  fileInputRef: React.RefObject<HTMLInputElement | null>;
}

const ModelCreatePost = forwardRef(
  ({ openModel, onClose, fileInputRef, isOpenEmoj, setIsOpenEmoj }: Props) => {
    const [addPost, { isLoading: loadingAddPost }] = useAddPostMutation();
      ...
      <div className="fixed inset-0 z-100 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
        <div className="animate-in zoom-in flex w-full max-w-[500px] flex-col rounded-xl bg-white shadow-2xl duration-200 dark:bg-zinc-900">
          ...
          <div className="p-4 pt-0">
            <div className="relative mb-4 flex items-center justify-between rounded-xl border border-gray-200 p-3 shadow-sm dark:border-zinc-800">
              <span className="text-sm font-bold">Thêm vào bài viết</span>
              <div className="flex w-auto items-center justify-end gap-1">
                ...
                {isOpenEmoj && (
                  <div
                    className="absolute right-18 bottom-10 z-100"
                ...
                <button
                  className="relative rounded-full border-none! bg-(--buttonColor)! p-2 text-yellow-500"
                  onClick={() => setIsOpenEmoj(pre => !pre)}
                  ref={ref}
                >
                  <Smile size={24} />
                </button>
            ...
            {selectPost ? (
              <button
                ...
                disabled={
                  (selectedImages.length === 0 && text.length === 0) ||
                  isLoading
                }
                className={`w-full cursor-not-allowed! py-2.5 ${(selectedImages.length > 0 || text.length > 0 || isLoading) && 'cursor-pointer! bg-blue-600! text-white! hover:bg-blue-700!'} rounded-lg border-none! font-bold shadow-md transition-all active:scale-[0.98] disabled:bg-gray-200! disabled:text-gray-400! dark:disabled:bg-zinc-800!`}
              >
                <div className="flex items-center justify-center gap-1">
                  {isLoading && <Spinner />}
                  {!isLoading && <span>Lưu</span>}
                </div>
              </button>
            ) : (
              <button
                ...
                className={`w-full cursor-not-allowed! py-2.5 ${(selectedImages.length > 0 || text.length > 0 || loadingAddPost) && 'cursor-pointer! bg-blue-600! text-white! hover:bg-blue-700!'} rounded-lg border-none! font-bold shadow-md transition-all active:scale-[0.98] disabled:bg-gray-200! disabled:text-gray-400! dark:disabled:bg-zinc-800!`}
              >
                <div className="flex items-center justify-center gap-1">
                  {loadingAddPost && <Spinner />}

                  {!loadingAddPost && <span>Đăng</span>}
                </div>
              </button>
            )}
          </div>
